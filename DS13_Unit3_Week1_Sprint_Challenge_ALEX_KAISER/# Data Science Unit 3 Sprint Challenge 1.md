...
A1. Well code should be readable and it's function should be easy to understand and follow. We all have our way of thinking, and certain ways to do something. The same is true with code, we wrote it, so we know what it does(at least at the beginning). A code review provides insides from another perspective, like a peer review it help creating a more understandable product.
When I review code, I pay particular attention to function and documentation. Highly functional code is good, but it's meaningless if anyone knows how it works in 2 weeks from now. Anyone who tries to build on the same code base should not have to decipher what was going on in someone elses head in order to use it. Clear instructions are key to anything that's meant to be reused.

A2. Containers could improve the situation by creating a clean self enclosed space to run an environment. Containers based on the same environment and the same packages can ensure the given programs work as intended, regardless from the clutter on the rest of the file system.
